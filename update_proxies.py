...
def _page(title: str, heading: str, subtitle: str, cards_html: str, updated: str) -> str:
    ...
    tcp_note = (
        '<div class="tcp-note">'
        '&#9888;&#65039; <b>Про пинг:</b> скорость измеряется от сервера GitHub (США) до прокси, '
        'а не от вас. Европейские прокси (~100&thinsp;ms) обычно быстрее для пользователей '
        'из России и СНГ, чем американские (~30&thinsp;ms). '
        'Зелёный значок ✅ означает что порт отвечает — '
        'итоговую работу проверяет сам Telegram при подключении.'
        '</div>'
    )
